...
#    (for psi*Am)
#  - center: starting value for the grid search for psi
...
sncftm.conf <- function(data, id, time, x, x.modelvars, x.linkfunction="identity", y,
  ...
  psi.esteq <- NULL
  psi.converge <- NULL
  estf.results <- suppressWarnings(optimx(0, estf.conf, dataprep.results=dataprep.env, blipfunction=blipfunction, method=c("nlminb")))
  if (estf.results$convcode!=0|estf.results$value>0.0001){
    estf.results1 <- suppressWarnings(optimx(0, estf.conf, dataprep.results=dataprep.env, blipfunction=blipfunction, method=c("nlm")))
  ...
  sncftm.boot <- function(i){
    ...
    estf.bootresults <- suppressWarnings(optimx(0, estf.conf, dataprep.results=dataprep.boot, blipfunction=blipfunction, method=c("nlminb")))
    if (estf.bootresults$convcode!=0|estf.bootresults$value>0.0001){
      estf.bootresults1 <- suppressWarnings(optimx(0, estf.conf, dataprep.results=dataprep.boot, blipfunction=blipfunction, method=c("nlm")))
